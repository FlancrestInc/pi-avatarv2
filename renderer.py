...
def read_state(config=None):
    ...
    try:
        ...
        if state not in config.states:
            return config.default_state, "Unknown state", None
        ...
        return state, detail, fps_override
    except Exception:
        return "offline" if "offline" in config.states else config.default_state, "State file unavailable", None
...
def load_all_animations(config, pygame_module):
    ...
    if config.default_state not in animations:
